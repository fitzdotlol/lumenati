...
// Custom controls
RAYGUIAPI int GuiUberNumber(Rectangle bounds, int *value, int minValue, int maxValue, bool editMode);
RAYGUIAPI int GuiUberNumberFloat(Rectangle bounds, float *value, float minValue, float maxValue, float increment, bool editMode);
RAYGUIAPI int GuiNewTabBar(Rectangle bounds, const char **text, int *icons, int *enabled, int count, int *active);                  // Tab Bar control, returns TAB to be closed or -1
...
static unsigned int guiIcons[RAYGUI_ICON_MAX_ICONS*RAYGUI_ICON_DATA_ELEMENTS] = {
    ...
    0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x3bb80000, 0x3bb83bb8, 0x00000000,      // ICON_230
...
static int FastGetGlyphIndex(Font font, int codepoint)
{
    return codepoint - 32;
}

static void FastDrawTextCodepoint(Font font, int codepoint, Vector2 position, float fontSize, Color tint)
{
    // Character index position in sprite font
    // NOTE: In case a codepoint is not available in the font, index returned points to '?'
    int index = FastGetGlyphIndex(font, codepoint);
        ...
        if (editMode)
        {
            ...
            if (CheckCollisionPointRec(mousePosition, textBounds))     // Mouse hover text
            {
                ...
                for (int i = textIndexOffset; i < textLength; i++)
                {
                    ...
                    codepointIndex = FastGetGlyphIndex(guiFont, codepoint);
    ...
    if ((text != NULL) && (text[0] != '\0'))
    {
        ...
        if ((guiFont.texture.id > 0) && (text != NULL))
        {
            ...
            for (int i = 0, codepointSize = 0; i < size; i += codepointSize)
            {
                ...
                int codepointIndex = FastGetGlyphIndex(guiFont, codepoint);
...
static float GetNextSpaceWidth(const char *text, int *nextSpaceIndex)
{
    ...
    for (int i = 0; text[i] != '\0'; i++)
    {
        if (text[i] != ' ')
        {
            ...
            index = FastGetGlyphIndex(guiFont, codepoint);
...
static void GuiDrawText(const char *text, Rectangle textBounds, int alignment, Color tint)
{
    ...
    for (int i = 0; i < lineCount; i++)
    {
        ...
        for (int c = 0, codepointSize = 0; c < lineSize; c += codepointSize)
        {
            ...
            int index = FastGetGlyphIndex(guiFont, codepoint);
                ...
                if ((codepoint != ' ') && (codepoint != '\t'))      // Do not draw codepoints with no glyph
                {
                    if (wrapMode == TEXT_WRAP_NONE)
                    {
                        ...
                        if (textSizeX > textBounds.width)
                        {
                            ...
                            {
                                FastDrawTextCodepoint(guiFont, codepoint, RAYGUI_CLITERAL(Vector2){ textBoundsPosition.x + textOffsetX, textBoundsPosition.y + textOffsetY }, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), GuiFade(tint, guiAlpha));
                            }
                            else if (!textOverflow)
                            {
                                ...
                                {
                                    FastDrawTextCodepoint(guiFont, '.', RAYGUI_CLITERAL(Vector2){ textBoundsPosition.x + textOffsetX + j, textBoundsPosition.y + textOffsetY }, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), GuiFade(tint, guiAlpha));
                                }
                            }
                        }
                        else
...
int GuiNewTabBar(Rectangle bounds, const char **text, int *icons, int *enabled, int count, int *active)
{
    float tabSpacing = 0;
    float tabWidth = bounds.width / count - tabSpacing;

    Rectangle tabBounds = { bounds.x, bounds.y, tabWidth, bounds.height };

    if (*active < 0) *active = 0;
    else if (*active > count - 1) *active = count - 1;

    int offsetX = 0;    // Required in case tabs go out of screen
    offsetX = (*active + 2)*tabWidth - GetScreenWidth();
    if (offsetX < 0) offsetX = 0;
    ...
    //--------------------------------------------------------------------
    for (int i = 0; i < count; i++) {
        tabBounds.x = bounds.x + (tabWidth + tabSpacing)*i - offsetX;
        ...
        // int icon = icons[i];
        int icon = enabled[i] ? icons[i] : 137;

        if (i == (*active)) {
            toggle = true;
            GuiToggle(tabBounds, GuiIconText(icon, text[i]), &toggle);
        } else {
            toggle = false;
            GuiToggle(tabBounds, GuiIconText(icon, text[i]), &toggle);
            if (toggle) *active = i;
...
int GuiUberNumber(Rectangle bounds, int *value, int minValue, int maxValue, bool editMode)
{
    static Vector2 startPoint = { 0 };
    static Vector2 lastPoint = { 0 };
    static float floatValue = 0.f;
    static bool editBox = false;
    ...
    {
        Vector2 mousePoint = GetMousePosition();
        Vector2 delta = Vector2Subtract(mousePoint, lastPoint);
        
        int hovering = CheckCollisionPointRec(mousePoint, bounds);
        if (hovering) {
            state = STATE_FOCUSED;
        }
        ...
        if (editMode && !editBox) {
            ...
            floatValue += delta.x / 2;
            tempValue = (int)floatValue;
            state = STATE_PRESSED;
        }

        Rectangle centerBounds = { bounds.x+11, bounds.y, bounds.width-22, bounds.height };

        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && !editBox && CheckCollisionPointRec(mousePoint, centerBounds)) {
            result = 1;
            ...
            Vector2 absDelta = Vector2Subtract(mousePoint, startPoint);
            if (fabsf(absDelta.x) < 0.1 && fabsf(absDelta.y) < 0.1) {
                editBox = true;
            }
        }
    }
    ...
    if (editBox) {
        result = GuiValueBox(bounds, NULL, &tempValue, minValue, maxValue, editMode);
        if (result) {
            editBox = false;
        }
    } else {
        ...
        const char *text = TextFormat("%d", tempValue);
        Color c = GetColor(GuiGetStyle(LABEL, TEXT + (state*3)));
        
        GuiDrawText(text, bounds, TEXT_ALIGN_CENTER, c);

        if (state == STATE_FOCUSED) {
            Rectangle buttonBoundsLeft = { bounds.x+1, bounds.y+1, 10, bounds.height-2 };
            Rectangle buttonBoundsRight = { bounds.x+bounds.width-11, bounds.y+1, 10, bounds.height-2 };
            int s = GuiGetStyle(BUTTON, BORDER_WIDTH);
            
            GuiSetStyle(BUTTON, BORDER_WIDTH, 0);
            
            if (GuiButton(buttonBoundsLeft, GuiIconText(ICON_ARROW_LEFT_FILL, NULL))) {
                --tempValue;
            }
            if (GuiButton(buttonBoundsRight, GuiIconText(ICON_ARROW_RIGHT_FILL, NULL))) {
                ++tempValue;
            }

            GuiSetStyle(BUTTON, BORDER_WIDTH, s);
        }
    }
...
int GuiUberNumberFloat(Rectangle bounds, float *value, float minValue, float maxValue, float increment, bool editMode)
{
    static Vector2 startPoint = { 0 };
    static Vector2 lastPoint = { 0 };
    static float floatValue = 0.f;
    static bool editBox = false;
    static char textValue[64] = { 0 };
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode)
    {
        ...
        if (editMode && !editBox) {
            lastPoint = mousePoint;
            floatValue += (delta.x / 2) * increment;
            tempValue = floatValue;
            state = STATE_PRESSED;
        }
        ...
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && !editBox && CheckCollisionPointRec(mousePoint, centerBounds)) {
            ...
            if (fabsf(absDelta.x) < 0.1 && fabsf(absDelta.y) < 0.1) {
                editBox = true;
                const char *buf = TextFormat("%f", tempValue);
                TextCopy(textValue, buf);
            }
        }
    }
    ...
    if (editBox) {
        result = GuiValueBoxFloat(bounds, NULL, textValue, &tempValue, editMode);
        if (result) {
            editBox = false;
        }
    } else {
        ...
        const char *text = TextFormat("%f", tempValue);
        Color c = GetColor(GuiGetStyle(LABEL, TEXT + (state*3)));
